...
BEGIN
    ...
    WHERE
        ...
        AND table_name NOT IN ('ativo', 'territorio_atendimento')
        LOOP
            ...
            EXECUTE format($body$ ALTER TABLE IF EXISTS %2$I.%1$I
                    ADD CONSTRAINT "%1$s_situacao_fk" FOREIGN KEY (situacao) REFERENCES catalogos.situacoes (codigo);
            $body$,
            mytables.table_name,
            mytables.table_schema);

            EXECUTE format($body$ ALTER TABLE IF EXISTS %2$I.%1$I
                    ADD CONSTRAINT "%1$s_proprietario_fk" FOREIGN KEY (proprietario) REFERENCES catalogos.proprietarios (codigo);
            $body$,
            mytables.table_name,
            mytables.table_schema);

            EXECUTE format($body$ ALTER TABLE IF EXISTS %2$I.%1$I
                    ADD CONSTRAINT "%1$s_responsavel_manutencao_fk" FOREIGN KEY (responsavel_manutencao) REFERENCES catalogos.gestores (codigo);
            $body$,
            mytables.table_name,
            mytables.table_schema);
