...
-- Ventosa (Respiro)
INSERT INTO sistema_agua.valvula_fluxo (tipo_ativo, data_instalacao, data_operacao, proprietario, responsavel_manutencao, observacoes, data_criacao, data_atualizacao, geom, rotacao_simbolo, diametro)
SELECT
    1 AS tipo_ativo,
...
-- Válvula de Controle de Nível (Altitude) & Válvula de Retenção
INSERT INTO sistema_agua.valvula_fluxo (tipo_ativo, data_instalacao, data_operacao, proprietario, responsavel_manutencao, observacoes, data_criacao, data_atualizacao, geom, rotacao_simbolo, diametro)
SELECT
    CASE WHEN funcaovalvula = 'ControleNivel' THEN 2
         WHEN funcaovalvula = 'Retencao' THEN 3
    END AS tipo_ativo,
    datainstalacao AS data_instalacao,
    dataoperacao AS data_operacao,
    CASE WHEN proprietario = 'EMPREENDIMENTOS' THEN 2 ELSE 1 END AS proprietario,
...
WHERE funcaovalvula = 'ControleNivel'
