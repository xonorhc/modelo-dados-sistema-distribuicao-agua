...
CREATE TABLE IF NOT EXISTS sistema_agua.macromedidor (
    id_objeto serial,
    id_ativo varchar(64) GENERATED ALWAYS AS ('MA-' || lpad(id_objeto::text, 5, '0')) STORED,
    diametro smallint,
    fabricante smallint,
    codigo_serie varchar(64),
    FOREIGN KEY (tipo_ativo) REFERENCES catalogo.tipo_macromedidor (codigo) ON UPDATE CASCADE ON DELETE RESTRICT,
...
CREATE INDEX ON sistema_agua.macromedidor USING gist (geom);
